...
$GLOBALS['sesId'] = $_SESSION['chat_ses_id_new'];
$GLOBALS['sesUsername'] = $_SESSION['sesUsername'];
        ...
        while($row=mysqli_fetch_array($sql_res))
        {
            ...
            <li class="person chatboxhead" id="chatbox1_<?php echo $username ?>" data-chat="person_<?php echo $id ?>" href="javascript:void(0);" onclick="javascript:chatWith('<?php echo $username ?>','<?php echo $id ?>','<?php echo $sesuserpic; ?>','<?php echo $onofst ?>')">
                <a href="javascript:void(0)">
                    <span class="userimage profile-picture min-profile-picture"><img src="<?php echo $config['base_url']; ?><?php echo $picname; ?>" alt="<?php echo $username ?>" class="bg-theme"></span>
            <span>
        ...
        <div class="user-bg">
            <div class="overlay-box">
                <div class="user-content"> <a href="javascript:void(0)">
                        <img class="thumb-lg img-circle" src="<?php echo $config['base_url']; ?><?php echo $picname; ?>" alt="<?php echo $username ?>"></a>
...
function chatfrindList($con,$config) {
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        <li class="person chatboxhead" id="chatbox1_<?php echo $username ?>" data-chat="person_<?php echo $id ?>" href="javascript:void(0)" onclick="javascript:chatWith('<?php echo $username ?>','<?php echo $id ?>','<?php echo $sesuserpic; ?>','<?php echo $onofst ?>')">
            <a href="javascript:void(0)">
                <span class="userimage profile-picture min-profile-picture"><img src="<?php echo $config['base_url']; ?><?php echo $picname; ?>" alt="<?php echo $username ?>" class="avatar-image is-loaded bg-theme" width="100%"></span>
                <span>
...
function get_all_msg($con,$config) {
    ...
    $sql = "select * from `".$config['db']['pre']."messages` where ((to_uname = '".$GLOBALS['sesUsername']."' AND from_uname = '".$_GET['client']."') OR (to_uname = '".$_GET['client']."' AND from_uname = '".$GLOBALS['sesUsername']."' )) and message_content!='' order by message_id DESC ";
...
function chatHeartbeat($con, $config)
{
    ...
    $sql = "select * from `".$config['db']['pre']."messages` where (to_uname = '".mysqli_real_escape_string($con,$GLOBALS['sesUsername'])."' AND recd = 0) and message_content!='' order by message_id ASC";
